...
export async function createStripePaymentIntent(orderId: string, groupId: string) {
  try {
    ...
    const isPaid = orderDetails.status === 'paid' || 
                  ...
                  orderDetails.payments?.some(p => p.status === 'paid' || p.status === 'completed');
    ...
    // Check if there's an existing initialized or pending Stripe payment intent
    ...
    // If there's an existing payment intent that's still valid, return that instead of creating a new one
